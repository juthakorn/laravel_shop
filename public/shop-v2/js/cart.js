...
class Cart{
    ...
    actionItem(target,quantity){
        ...
        var product_attr_id = target.closest('tr').find('[name="data_product"]').attr('data-attr-id'); 
        
        if(product_attr_id !== undefined){
            var product = this.product_data[product_attr_id];
            var chk = Cart.check_stock(target,quantity,product);
    ...
    static check_payment(price){
        ...
        if ($('select[name="payment"]').val().length > 0 && $('select[name="delivery"]').val().length > 0) {    
            if($('select[name="payment"]').val() === '2'){
                var percen = 3; //เก็บปลายทางเคอรี่คิด 3% จากราคาทั้งหมด
                var temp_result =  price * percen/100;
                var scrap = temp_result%10;

                var result = temp_result- scrap;
                if(scrap > 5){ 
                 result = result + 10;
                }
...
//                }
    ...
    static check_stock(target,quantity,product){
        console.log(quantity);
         console.log(product);
        var main = target.closest('.main-form-num');
...
//       
        $('#table-cart tbody tr').each(function(event){ 
            var temp_price = $(this).find(".item_price").attr("data-value");
            var qty = $(this).find('input[name="data_atrr_qty"]').val();
            var Total_item_price = temp_price*qty;
        ...
        if(!isNaN(discount)){
            var num_discount = (Total*discount/100);
